 ...
 */
@ApiModel("模型-黑名单信息")
public class CMCdnBlackReferer implements Serializable {
    ...
    @ApiModelProperty(value = "referer黑名单列表", notes = "默认[]")
    @JSONField(name = "allow_list")
    private List<String> allowList;

    @ApiModelProperty(value = "是否追加referer黑名单列表", notes = "是否在原来的黑名单列表基础上追加黑名单，取值 1:追加, 0:覆盖，不传默认0覆盖。" +
            "当传入的值为1的时候，将当前传入的allow_list追加到已有配置的黑名单列表。当传入值0或者不传值的时候，当前传入的allow_list覆盖已有配置的黑名单列表")
